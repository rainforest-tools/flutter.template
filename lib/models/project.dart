...
final projects = <Project>[
  new FrontendProject(
      id: UniqueKey(),
      timestamp: [new DateTime.utc(2019, 12)],
  ...
  new FrontendProject(
      ...
      name: 'APCOM 議程網頁',
      description:
          'Asian Pacific Congress on Computational Mechanics (APCOM) 會議議程表，考量與會者會有行動端的需求，特別優化 RWD 的部分。',
      ...
      // TODO: imporvement is required
      tags: [tags['VUE']!, tags['FRONTEND']!, tags['RWD']!]),
  new Project(
      id: UniqueKey(),
      timestamp: [
  ...
  new Project(
      ...
      name: 'BIM 管理平台',
      description:
          '整合 BIM(Building Information Modeling) 資訊及工程各階段文檔、數據，並進一步視覺化。',
      imageUrl: 'assets/images/projects/BIM 管理平台.png',
      // TODO: imporvement is required
      tags: [tags['REACT']!, tags['FRONTEND']!]),
  new FrontendProject(
      id: UniqueKey(),
      timestamp: [
        new DateTime.utc(2019, 7),
...
class FrontendProject extends Project {
  ...
  FrontendProject({
    ...
    required List<DateTime> timestamp,
...
class Tag {
  ...
  bool isSelected;
  IconData? icon;
  Tag({required this.name, this.isSelected = false, this.icon});
}
...
class TagsNotifier extends ChangeNotifier {
  ...
  void _setFiltedProjects() {
    _filtedProjects = _selectedTags.length == 0
        ? projects
        : projects
            .where((project) =>
                project.tags
                    .toSet()
                    .intersection(_selectedTags.toSet())
                    .length ==

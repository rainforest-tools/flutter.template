...
class ProjectsPage extends StatefulWidget {
  ProjectsPage({Key? key, this.mode = Mode.GRID, required this.projectId})
      : super(key: key);
...
class _ProjectsPageState extends State<ProjectsPage>
  ...
  void initState() {
    ...
    var initialItem = tagsNotifier.filtedProjects.indexWhere(
        (project) => getIdFromUniqueKey(project.id) == widget.projectId);
    ...
    _tween = new Tween<double>(begin: 0, end: 1).animate(new CurvedAnimation(
  ...
  Widget build(BuildContext context) {
    ...
    return DefaultLayout(
        appBar: AppBar(
          title: Text('Projects'),
          actions: <Widget>[Hero(tag: 'socialLinks', child: new SocialBar())],
        ),
        body: Padding(
                      ...
                      Expanded(
                        child: Container(
                          ...
                          child: Wrap(
                              ...
                              children: tagsNotifier.selectedTags
                                  .map((tag) => ActionChip(
                                      ...
                                      label: Text(tag.name),
                                      onPressed: () =>
                                          _onChipPressed(tag, tagsNotifier)))
                                  .toList()),
                      ...
                      FlatButton(
                          ...
                          onPressed: () {
                            showSearch(
                                context: context,
                                delegate:
                                    ProjectSearch(tagsNotifier.filtedProjects));
                          })
                    ],
                  ),
                ),
  ...
  Widget _getProjectsPageBody(BuildContext context) {
    ...
    switch (widget.mode) {
      case Mode.DETAIL:
        if (!ResponsiveHelper().isXsmall(context))
          return Row(
                  ...
                  child: ListWheelScrollView.useDelegate(
                    ...
                    childDelegate: ListWheelChildBuilderDelegate(
                        ...
                        builder: (context, index) {
                          final project =
                              tagsNotifier.filtedProjects[index.abs()];

                          return Hero(
                            tag:
                                'projectCard ${getIdFromUniqueKey(project.id)}',
                            child: AspectRatio(
                        ...
                        child: Column(
                            ...
                            children: <Widget>[
                              ...
                              Padding(
                                ...
                                child: Text(
                                    _selectedProject?.timestamp.length == 1
                                        ? new DateFormat('yyyy 年 MM 月').format(
                                            _selectedProject!.timestamp.first)
                              ...
                              Wrap(
                                  spacing: 10,
                                  direction: Axis.horizontal,
                                  children: _selectedProject!.tags
                                      .map((tag) => ActionChip(
                                          avatar: tag.icon != null
                                              ? Icon(tag.icon)
                                              : null,
        ...
        return ListView(children: <Widget>[
          ...
          Padding(
            ...
            child: Text(
                _selectedProject?.timestamp.length == 1
                    ? new DateFormat('yyyy 年 MM 月')
                        .format(_selectedProject!.timestamp.first)
                    : '${new DateFormat('yyyy 年 MM 月').format(_selectedProject!.timestamp.first)} - ${new DateFormat('yyyy 年 MM 月').format(_selectedProject!.timestamp.last)}',
                style: Theme.of(context).textTheme.subtitle2),
          ),
          Text(_selectedProject!.description,
          ...
          Wrap(
              ...
              children: _selectedProject!.tags
                  .map((tag) => ActionChip(
                      ...
                      backgroundColor:
                          tag.isSelected ? Theme.of(context).accentColor : null,
                      label: Text(tag.name),
      ...
      default:
        return GridView.builder(
            gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                crossAxisCount:
                    ResponsiveHelper().valueGiver<int>(context, 1, 1, 2, 3, 5)!,
                mainAxisSpacing: 30,
                crossAxisSpacing: 30,
                childAspectRatio: 1 /
                    ResponsiveHelper()
                        .valueGiver(context, 1.5, 1.5, 1.4, 1.4, 1.4)!),
            itemCount: tagsNotifier.filtedProjects.length,
            itemBuilder: (_, index) => GestureDetector(
                onTap: () => routesNotifier.router.navigateTo(context,
                    '/projects/${getIdFromUniqueKey(tagsNotifier.filtedProjects[index].id)}?mode=detail'),
                child:
                    ProjectCard(project: tagsNotifier.filtedProjects[index])));
    }
  }

  void _onChipPressed(Tag tag, TagsNotifier tagsNotifier) {
    if (tagsNotifier.selectedTags.contains(tag))
      tagsNotifier.remove(tag);
    else
      tagsNotifier.add(tag);
...
class ProjectSearch extends SearchDelegate<Project> {
  ...
  Widget buildResults(BuildContext context) {
    final tagsNotifier = Provider.of<TagsNotifier>(context);
    final results = tagsNotifier.filtedProjects
        .where((project) =>
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        Flexible(child: _getResultsListView(context, results)),
      ],
    );
  }

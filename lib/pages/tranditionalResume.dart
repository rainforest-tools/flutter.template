...
class _TranditionalResumeState extends State<TranditionalResume> {
  ...
  Widget build(BuildContext context) {
    ...
    final experienceTimeline = new Timeline(
      ...
      descriptionStyle: Theme.of(context).textTheme.overline!.apply(
          fontSizeFactor:
              ResponsiveHelper().valueGiver(context, 0.8, 1, 1, 1, 1)!),
    );
    ...
    return DefaultLayout(
      appBar: AppBar(
        ...
        actions: <Widget>[_createDraggable(SocialBarPosition.APPBAR, context)],
      ),
      body: Stack(
        children: <Widget>[
          Align(
            ...
            child: Padding(
              padding: EdgeInsets.symmetric(
                  vertical: ResponsiveHelper().isXsmall(context)
                      ? 0
              ...
              child: RepaintBoundary(
                  ...
                  child: Transform.scale(
                    scale:
                        ResponsiveHelper().valueGiver(context, 1, 1, 1, 1, 1)!,
                    child: AspectRatio(
                      aspectRatio: 210 / 297,
                      child: Card(
                          child: Padding(
                                                      ...
                                                      itemBuilder:
                                                          (_, index) =>
                                                              ListTile(
                                                                ...
                                                                leading: Hero(
                                                                    tag:
                                                                        'skill_${skills[index].name}_icon',
                                                                    child: Icon(
                                                                        skills[index]
                                                                            .icon)),
                                                                title: Align(
                                                                  ...
                                                                  child:
                                                                    ...
                                                                    child: Hero(
                                                                      tag:
                                                                          'skill_${skills[index].name}_name',
                                                                      child: Text(
                                                                          skills[index]
                                                                              .name,
                                                                          style: Theme.of(context)
                                                                ...
                                                                subtitle: Align(
                                                                  ...
                                                                  child: Hero(
                                                                      tag:
                                                                          'skill_${skills[index].name}_description',
                                                                      child: getDescriptionWidget(
                                                                          ...
                                                                          Theme.of(context)
                                                                              .textTheme
                                                                              .overline
                                                                              ?.apply(fontSizeFactor: ResponsiveHelper().valueGiver(context, 0.8, 1, 1, 1, 1)!))),
                                                                ),
                                                              )),
                                                ),
                                              ],
          ...
          ...alignments
              .map((e) => mapSocialBarPositionWithAlignment(e))
              .map((e) => _createDraggable(e!, context))
              .where((c) => c != null)
              .toList()
        ],
      ),
  ...
  Widget _createDraggable(SocialBarPosition position, BuildContext context) {
    ...
    final alignment = alignments.firstWhere((element) =>
        position.toString().split('.').last.toLowerCase() ==
        element.toString().toLowerCase());
    if (alignment == null) {
      switch (position) {
        case SocialBarPosition.APPBAR:
          return settingsNotifier.settings.socialBarPosition == position
              ? Draggable(
                  data: position,
                  feedback: Hero(tag: 'socialLinks', child: new SocialBar()),
                  childWhenDragging: _createPlaceHolder(
    ...
    return Padding(
      ...
      child: Align(
          alignment: alignment,
          child: settingsNotifier.settings.socialBarPosition == position
              ? Draggable(
                  data: position,
                  feedback: Hero(
                      tag: 'socialLinks',
  ...
  Widget _createDragTarget(BuildContext context, SocialBarPosition position) {
    ...
    return DragTarget<SocialBarPosition>(
      ...
      builder: (context, candidateData, rejectedData) {
        if (_isDragging)
          return _createPlaceHolder(context, Theme.of(context).accentColor);
        return Container();
      },
    );
  }
